...
RUN tar -xvf jdk-8u45-linux-x64.tar.gz -C /opt && rm jdk-8u45-linux-x64.tar.gz

RUN chown -R root: /opt/jdk1.8.0_45
RUN alternatives --install /usr/bin/java java /opt/jdk1.8.0_45/bin/java 1
RUN alternatives --install /usr/bin/javac javac /opt/jdk1.8.0_45/bin/javac 1
RUN alternatives --install /usr/bin/jar jar /opt/jdk1.8.0_45/bin/jar 1
